...
export const getMoviesFromTmdb = async (TMDB_KEY, minimumRating, genreId) => {
  ...
  const fetchUrl = genreId ? `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_KEY}&with_genres=${genreId}&vote_count.gte=500&sort_by=popularity.desc&vote_average.gte=${minimumRating}` :
    `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_KEY}&vote_count.gte=500&sort_by=popularity.desc&vote_average.gte=${minimumRating}`;
